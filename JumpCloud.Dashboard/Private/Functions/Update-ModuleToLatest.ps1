...
Function Update-ModuleToLatest
{
    ...
    Try
    {
        ...
        If (-not [System.String]::IsNullOrEmpty($ModuleInfo))
        {
            ...
            {
                $LocalInstalledVersion = If (-not [System.String]::IsNullOrEmpty($ModuleData.LocalInstalled)) { $ModuleData.LocalInstalled.Version -split '-' } Else { 0.0.0 }
                $PSGalleryVersion = $ModuleData.PSGallery.Version -split '-'
                If ([Version]$PSGalleryVersion[0] -gt [Version]$LocalInstalledVersion[0])
                {
                    ...
                    @{Name = 'STATUS'; Expression = { 'An update is available for the "' + $ModuleData.PSGallery.Name + '" PowerShell module.' } } `
                        , @{Name = 'INSTALLED VERSION(S)'; Expression = { If (-not [System.String]::IsNullOrEmpty($ModuleData.LocalInstalled)) { $ModuleData.LocalInstalled | ForEach-Object { $_.Version + ' (' + (Get-Date $_.PublishedDate).ToString('MMMM dd, yyyy') + ')' } } } } `
                    ...
                    If ($UserInput -eq 'Y' -or $Force -eq $true)
                    {
                        ...
                        If (-not [System.String]::IsNullOrEmpty($ModuleData.LocalImported.Name))
                        {
                            ...
                            Get-Module -Name:($ModuleData.LocalImported.Name) -ListAvailable -All | Remove-Module -Force
                        }
                        ...
                        Import-Module -Name:($ModuleData.PSGallery.Name) -Version:($PSGalleryVersion[0]) -Force -Scope:('Global')
